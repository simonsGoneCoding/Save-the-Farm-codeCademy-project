...
function moveCupboard(){
    ...
    // $ready_to_work decides if the player is prepared to move the cupboard. It should be TRUE if they player is not hungry and they’re wearing contacts and they are not wearing glasses and they do not need to pee.
    $ready_to_work = !$is_hungry && $wearing_contacts && !$wearing_glasses && !$needs_to_pee;
    
    //logics to determine location and possibility to move cupboard
    if($location !== "kitchen"){
        echo "You don't see a cupboard here!\n"; 
    }elseif($location === "kitchen" && $moved_cupboard){
